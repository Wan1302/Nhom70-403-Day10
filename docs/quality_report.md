...
- R8 và R9 đã được xác nhận bằng run `r8r9` trên `data/raw/policy_export_r8r9.csv`: `quarantine_r8r9.csv` chứa 2 dòng với reason `missing_exported_at` và `chunk_text_too_short` (`chunk_length=3`), `manifest_r8r9.json` ghi `raw_records=4, cleaned_records=2, quarantine_records=2`.
